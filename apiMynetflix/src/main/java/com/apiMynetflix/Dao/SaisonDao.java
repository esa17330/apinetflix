...
public class SaisonDao {
	...
	public List<Saison> listerSaison(int idserie) throws DaoException {
		...
		try {
			...
			try (PreparedStatement pprst = cnx.prepareStatement(request)) {
				...
				while (rs.next()) {
					listesaison.add(new Saison(rs.getInt("numero"), rs.getString("resume"),
	...
	public void modifierSaison(Saison saison) throws DaoException {

		String request = "UPDATE saison	\r\n" + "SET numero=?, resume=?, anneeDiffusion=?, idStatut=?, idSerie=?,"
				+ "WHERE id = ?";
		try {
			...
			try (PreparedStatement pprst = cnx.prepareStatement(request)) {
				...
				pprst.setObject(3, saison.getAnneeDiffusion(), Types.NUMERIC);
				pprst.setObject(4, saison.getIdStatut(), Types.INTEGER);
				pprst.setObject(5, saison.getIdSerie(), Types.INTEGER);
				pprst.setObject(6, saison.getId(), Types.NUMERIC);
				pprst.executeUpdate();
				cnx.commit();
				pprst.close();
				cnx.close();
